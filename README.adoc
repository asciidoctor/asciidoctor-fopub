...
By default, XSL processors fetch all the necessary resources from the internet.
Since these files are large and reference many other files, fetching them from the internet is exceptionally slow and uses unnecessary network bandwidth.
The +fopdf+ tool carefully ensures that the processor has all the files it needs (on the first run) and thus keeps it from reaching out to the internet while performing the conversion.
