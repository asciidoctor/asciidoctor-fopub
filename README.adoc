...
. Copy the +docbook-xsl+ directory from the fopdf install directory to your documentation path, [file]_/path/to/custom/fopdf_.
+
...
. Then you specify this directory when you launch the output generation :
 ...
 $ /path/to/fopdf/fopdf -t /path/to/custom/fopdf/docbook-xsl sample.xml
 
=== fopdf option flags

-t <path>::
  tells fopdf which docbook-xsl directory to use (optional, defaults to location inside fopdf installation)
-f [pdf|ps|fo]::
  tells fopdf which output to produce (optional, defaults to +pdf+)
-h::
  prints usage
